...
void NetworkManager::begin(const String& ssid, const String& pass, int32_t tzOffset) {
    _ssid = ssid;
    _pass = pass;
    _tzOffset = tzOffset;
    ...
    // 1. Disable modem sleep (drastically improves reliability and perceived signal strength)
    WiFi.setSleep(false); 
    // 2. Explicitly force maximum TX power (19.5 dBm is the standard max for ESP32)
...
void NetworkManager::startAP() {
    ...
    LOG_I("WIFI", "AP started: %s (IP: %s)", _apName.c_str(), WiFi.softAPIP().toString().c_str());
}
...
NetState NetworkManager::getState() const {
    return _state;
}
...
String NetworkManager::getAPName() const {
    return _apName;
}
