...
void OTAManager::update(uint32_t nowMs) {
    ...
    if (nowMs - _lastCheckMs >= OTA_CHECK_INTERVAL_MS) {
        checkForUpdate();
    }
}
...
bool OTAManager::performUpdate() {
    ...
    switch (ret) {
        case HTTP_UPDATE_FAILED:
            _lastError = String("Update failed: ") +
...
bool OTAManager::fetchLatestRelease() {
    ...
    if (_updateAvailable) {
        LOG_I("OTA", "Current: %s Latest: %s -> Update available!", FIRMWARE_VERSION, _latestVersion.c_str());
    } else {
        LOG_I("OTA", "Current: %s Latest: %s -> Up to date", FIRMWARE_VERSION, _latestVersion.c_str());
    }
